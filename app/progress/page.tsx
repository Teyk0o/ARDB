...
import { FaSearch, FaCheck, FaTimes, FaFileExport, FaFileImport, FaCheckCircle, FaFilter, FaCloud, FaInfoCircle } from 'react-icons/fa';
...
export default function ProgressPage() {
  ...
  const handleSelectAll = () => {
    ...
    if (activeTab === 'all' || activeTab === 'projects') {
      filteredProjects.forEach((p) => {
        if (!isCompleted('projects', p.id)) {
          toggleCompletion('projects', p.id);
        }
      });
    }
  ...
  const totalAll = totalQuests + totalProjects + totalWorkshops;
              ...
              {(activeTab === 'all' || activeTab === 'projects') && (
                <>
                  {filteredProjects.map((project) => (
                    <CompletionItem
                      key={project.id}
                      label={project.name[language] || project.name.en}
                      description={project.description?.[language] || project.description?.en}
                      completed={isCompleted('projects', project.id)}
                      onToggle={() => toggleCompletion('projects', project.id)}
                      color="#3b82f6"
                      type="project"
                      language={language}
                    />
